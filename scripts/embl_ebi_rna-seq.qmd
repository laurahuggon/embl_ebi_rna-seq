...
Not all mouse genes in the genome will be expressed in these samples. Here, we will use a minimal threshold that if a gene does not have more than 5 counts total across all samples, it is filtered out (as it is not enough data to be able to do anything with).
...
Removes genes with total counts less than or equal to 5 across all samples.

```{r}
# Extract counts matrix
counts_matrix = txi$counts

# Calculate total counts for each gene (i.e. sum across columns)
total_count = rowSums(counts_matrix)

# Filter genes
# Returns a logical vector (TRUE and FALSE) values
filtered_genes = total_count > 5

# Update txi to include only the filtered genes
# If an element is filtered genes is TRUE, the row is kept
txi$abundance = txi$abundance[filtered_genes, ]
txi$counts = txi$counts[filtered_genes, ]
txi$length = txi$length[filtered_genes, ]
```

Count the number of rows (genes) after filtering.

```{r}
nrow(txi$counts) # 20,383 genes
```
...
```{r}
dds # dds contains 20,383 genes and 9 samples
```
...
enrichplot::dotplot(gp_mod_bp_enrich, title="Gene Ontology:Biological Process")
```
...
```{r}
length(brainlung) # 378 genes are downregulated in LG compared to BR
```

```{r}
length(brainliver) # 731 genes are downregulated in LV compared to BR
```
...
length(brain_enriched) # 221 genes are downregulated in LG and LV compared to the brain
```
...
barplot(gp_mod_bp_enrich, showCategory = 10, font.size = 12, title="Gene Ontology:Biological Process") + 
